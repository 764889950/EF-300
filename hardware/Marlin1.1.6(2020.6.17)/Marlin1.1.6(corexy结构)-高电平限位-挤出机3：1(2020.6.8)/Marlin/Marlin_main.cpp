...
//M870: turn off machine，新增
inline void gcode_M870()
{
  if (parser.seenval('S')) {
    const uint16_t temp = parser.value_celsius();
    turn_off_time=temp;
    //customizedSerial.println(temp);
  }
}
  ...
  switch (parser.command_letter) {
    ...
    case 'M': switch (parser.codenum) {
      ...
      case 84: // M84: Disable all steppers or set timeout
        //static char print_end_flag=0;//新增，位于G28,M84
        if( print_end_flag==1)
        {
          //SERIAL_PROTOCOLPGM("M84");
          //此处加入语音控制
          ...
          turn_off_flag=1;//自动关机标识，允许自动关机，新增
        }
        gcode_M18_M84();
        break;
      ...
      case 870: // M870: turn off machine，新增2020.6.11
        gcode_M870();
      break;     
    }
    break;
...
void loop() {  
  ...
  {
    t1++;
    if(t1==65535)//t1==65535大概需要5S钟
    {
      t2++;
      if(t2==turn_off_time*12)
      {
        //SERIAL_ECHOLNPGM("Compiled_lyh");
        digitalWrite(pin_turn_on_off_control,HIGH);//执行关机操作
        turn_off_flag=0;
        t2=0;
      }
      t1=0;
  ...
  if(!(digitalRead(pin_turn_on_off_get)))//读取按键开关输入脚,如果读取到低电平
  {
      digitalWrite(pin_turn_on_off_control,HIGH);//执行关机操作
  }
